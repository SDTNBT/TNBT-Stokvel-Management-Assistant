...
import { auth } from '../services/firebase'; // Ensure this points to your team's Firebase config file
import './Login.css';
...
export const LoginPage = () => {
  ...
  // 2. NEW: Immediately wipe the history state so it never triggers again
  useEffect(() => {
    if (location.state?.justSignedUp) {
      // replace: true silently overwrites the current history entry without reloading
      navigate(location.pathname, { replace: true, state: {} });
    }
  }, [location, navigate]);

  // Existing useEffect — clears session on load
  useEffect(() => {
  ...
  // New useEffect — auto-dismisses the welcome modal after 8 seconds
  useEffect(() => {
    if (!showModal) return;
  ...
  const syncWithBackend = async (firebaseToken, firstName='', lastName='') => {
    ...
    if (response.ok) {
      // Success! Save the token for API calls
      localStorage.setItem('token', firebaseToken);
      
      // CRITICAL: Save the returned MongoDB user data so the Dashboards can read it
  ...
  const handleGoogleLogin = async () => {
    ...
    setLoading(true);
    try {
      // 1. Open the Google popup and let Firebase handle the heavy lifting
      const provider = new GoogleAuthProvider();
      const result = await signInWithPopup(auth, provider);

      // 2. Extract the user's First and Last name from the raw Google JSON object
      const details = getAdditionalUserInfo(result);
      const googleFirstName = details.profile.given_name;
      const googleLastName = details.profile.family_name;

      // 3. Grab the secure Firebase Token
      const token = await result.user.getIdToken();
      
      // 4. Send the token and details to our Helper function to save in the database
  ...
  const handleManualLogin = async (e) => {
    ...
    setLoading(true);
    try {
      // 1. Tell Firebase to verify the email and password
      const userCredential = await signInWithEmailAndPassword(auth, email, password);
      
      // 2. Grab the secure Firebase Token
      const token = await userCredential.user.getIdToken();
      
      // 3. Send token to backend via the Helper function
      await syncWithBackend(token); 
    } catch (err) {
      console.error("Manual Login Error:", err);
      // Firebase will automatically throw a specific error here if the password is wrong or user doesn't exist
      ...
      <article className="login-card">
        ...
        <section className="login-google-wrapper" aria-label="Sign in options">
          {loading ? (
            ...
            >
              <img src="public/images/icons8-google-48.png" alt="Google logo" style={{ width: '20px', verticalAlign: 'middle', marginRight: '10px' }}/>
